...
public static class ApiKeyGuard
{
    ...
    private const string Hint =
        "Configure a chave que a Custódia exige de quem a chama via variável de ambiente ApiKey__Key " +
        "(Docker/produção) ou via dotnet user-secrets set \"ApiKey:Key\" \"<chave>\" --project src/Custodia.API " +
        "(dev local).";
}
