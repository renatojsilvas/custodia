...
public sealed class RabbitMqConnectionProvider : IAsyncDisposable
{
    ...
    private const string ClientProvidedName = "custodia-consumidor";
    ...
    public async Task<IConnection> ObterConexaoAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    await RabbitMqTopologia.DeclararAsync(channel, ct);
                }
    ...
    private void RegistrarSucesso()
    {
        if (_ultimaConexaoOk != true)
        {
            _logger.LogInformation(
                "Conexão com o RabbitMQ estabelecida em {Host}:{Port}, topologia da custódia declarada.",
                _host, _port);
        }
    ...
    private void RegistrarFalha(Exception ex)
    {
        ...
        {
            _logger.LogError(ex, "Falha ao conectar ao RabbitMQ em {Host}:{Port} ou ao declarar a topologia.", _host, _port);
        }
